...
def execute(filters=None):
	...
	if filters.group_by == "Item Code":
		for d in data:
			frac, whole = math.modf(d[4])
			item_doc = frappe.get_doc("Item" , d[0])
			if frac:
				
				streps = frac * int(item_doc.uoms[1].no_of_streps)
				d[4] = f' {whole} and {round(streps)} strep  '
				if not whole:
					d[4] = f' {round(streps)} strep  '

		columns[4]['fieldtype'] = "Data"
		columns[4]['width'] = "150"
