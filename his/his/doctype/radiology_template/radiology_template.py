...
def change_test_code_from_template(template_code, doc):
	...
	if frappe.db.exists({'doctype': 'Item', 'item_code': template_code}):
		...
		frappe.db.set_value('Radiology Template', doc.name, 'template_code', template_code)
		frappe.db.set_value('Radiology Template', doc.name, 'template', template_code)
		rename_doc('Radiology Template', doc.name, template_code, ignore_permissions=True)
