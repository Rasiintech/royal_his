	...
	def on_trash(self):
		...
		if self.item:
			try:
				...
				frappe.throw(_('Not permitted. Please disable the EGD Template'))
...
def create_item_from_template(doc):
	...
	frappe.db.set_value('EGD Templete', doc.name, 'item', item.name)
...
def change_test_code_from_template(template_code, doc):
	...
	if frappe.db.exists({'doctype': 'Item', 'item_code': template_code}):
		frappe.throw(_('EGD Item {0} already exist').format(template_code))
	else:
		rename_doc('Item', doc.name, template_code, ignore_permissions=True)
		frappe.db.set_value('EGD Templete', doc.name, 'template_code', template_code)
		frappe.db.set_value('EGD Templete', doc.name, 'template', template_code)
		rename_doc('EGD Templete', doc.name, template_code, ignore_permissions=True)
