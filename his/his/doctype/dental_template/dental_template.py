		...
		if self.change_in_item and self.is_billable and self.item:
			...
			if not item_price:
				if self.lab_test_rate and self.lab_test_rate > 0.0:
					...
					make_item_price(self.template_code, price_list_name, self.lab_test_rate)
			else:
	...
	def on_trash(self):
		...
		if self.item:
			try:
				...
				frappe.throw(_('Not permitted. Please disable the Radiology Template'))
...
def create_item_from_template(doc):
	...
	frappe.db.set_value('Radiology Template', doc.name, 'item', item.name)
...
def change_test_code_from_template(template_code, doc):
	...
	if frappe.db.exists({'doctype': 'Item', 'item_code': lab_test_code}):
		frappe.throw(_('Dental Item {0} already exist').format(lab_test_code))
	else:
		rename_doc("Item", doc.item, lab_test_code, ignore_permissions=True)
		frappe.db.set_value(
			"Dental Template", doc.name, {"lab_test_code": lab_test_code, "lab_test_name": lab_test_code}
		)
		rename_doc("Dental Template", doc.name, lab_test_code, ignore_permissions=True)
